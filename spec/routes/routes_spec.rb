...
feature 'REST endpoints' do
  ...
  scenario 'Root should redirect to API root' do
    visit '/'
    expect_content('Welcome to the Recipe Reader Ruby on Rails API Backend')
  end
  ...
  scenario 'Return a specific recipe from /api/v1/recipes/:id' do
    ...
    expect_content('Squid, chickpea & chorizo salad')
    expect_content('224260')
    visit '/api/v1/recipes/519078'
    expect_content('Easy Chicken Pot Pie')
    expect_content('519078')
  end

  scenario 'Return ingredients from /api/v1/recipes/22460/ingredients' do
  ...
  scenario 'Return ingredients from /api/v1/recipes/519078/ingredients' do
    visit '/api/v1/recipes/519078/ingredients'
    expect_content('potato')
    expect_content('11362')
    expect_content('236.5')
    expect_content('ml')
    expect_content('us_amount')
    expect_content('cup')
    expect_content('potatoes-yukon-gold.png')
    expect_content('3/4 cup sliced carrot')
    expect_content('1/2 cup butter')
    expect_content('2/3 cup diced onion')
    expect_content('1 1/4 teaspoon salt')
    expect_content('1/2 teaspoon ground pepper')
    expect_content('1/4 teaspoon dried thyme')
    expect_content('1/4 teaspoon poultry seasoning')
    expect_content('3 cups shredded chicken from a rotisserie chicken')
  end
